...
{- Update Field tree and script in treebank to make all slashes to '/'.
 - Slashes in CCG parsing trees are '\' and '/', while they have types '.', '#', 'x' or '*'.
 - Slashes in pure-CCG parsing trees are '/', and they have no types.
 -}
purifySlashInTreeAndScript :: IO ()
purifySlashInTreeAndScript = do
    confInfo <- readFile "Configuration"                                        -- Read the local configuration file
      ...
      then putStrLn "No sentence is designated."
        ...
        if contOrNot == "c"
          then do
            ...
            if (rowsParsed == [])
              then putStrLn "purifySlashInTreeAndScript: No sentence is asked to do this operation."
              else do
                let rows2Memory = map (\row -> (fst3 row, (readTrees . snd3) row, (readScripts . thd3) row)) rowsParsed     -- [(SendIdx, [Tree], [Script])]
--                putStrLn "purifySlashInTreeAndScript: Original trees and scripts: "
--                mapM_ (\row -> (putStr "(" >>= \_ -> putStr (show (fst3 row)) >>= \_ -> putStr "," >>= \_ -> showTrees (snd3 row) >>= \_ -> putStr "," >>= \_ -> showScripts (thd3 row) >>= \_ -> putStrLn ")")) rows2Memory

                let rows2Memory' = map (\row -> (fst3 row, (purifySlashInTree4ASent . snd3) row, (purifySlashInScript4ASent . thd3) row)) rows2Memory
--                putStrLn "purifySlashInTreeAndScript: Slash purified trees and scripts: "
--                mapM_ (\row -> (putStr "(" >>= \_ -> putStr (show (fst3 row)) >>= \_ -> putStr "," >>= \_ -> showTrees (snd3 row) >>= \_ -> putStr "," >>= \_ -> showScripts (thd3 row) >>= \_ -> putStrLn ")")) rows2Memory'
...
{- For a sentence, parsing result is [Tree], namely [(ClauIdx, [PhraCate])].
 - This function is to purify slashes in all PhraCate values.
 -}
purifySlashInTree4ASent :: [Tree] -> [Tree]
purifySlashInTree4ASent [] = []
purifySlashInTree4ASent [(clauIdx, pcs)] = [(clauIdx, map purifySlashInPhrase pcs)]
purifySlashInTree4ASent (t:ts) = purifySlashInTree4ASent [t] ++ purifySlashInTree4ASent ts

{- For a sentence, parsing script is [Script], namely [(ClauIdx, [[Rule]], [BanPCs])].
 - This function is to purify slashes in all PhraCate values in every BanPCs.
 -}
purifySlashInScript4ASent :: [Script] -> [Script]
purifySlashInScript4ASent [] = []
purifySlashInScript4ASent [(clauIdx, ruleSets, banPCSets)] = [(clauIdx, ruleSets, map (map purifySlashInPhrase) banPCSets)]
purifySlashInScript4ASent (s:ss) = purifySlashInScript4ASent [s] ++ purifySlashInScript4ASent ss

{- For a phrase, it is represented as ((Start, Span), [(Category, Tag, Seman, PhraStru, Act)], SecStart), here CTSPA has only one member.
 - This function is to check Field Category, and change slashes in this Field into '/'.
 -}
purifySlashInPhrase :: PhraCate -> PhraCate
purifySlashInPhrase ((start, span), [(cate, tag, seman, phraStru, act)], secStart)
    = ((start, span), [(purifySlash cate, tag, seman, phraStru, act)], secStart)
