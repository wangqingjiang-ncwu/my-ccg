...
module Rule (
    rules,    -- [(Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)]
    ruleTags, -- [Tag]
    semComb,  -- Seman -> Seman -> Seman
    appF,     -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    appB,     -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    comFh,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    comFh2,   -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    comBh,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    comFc,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    comBc,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    raiFh,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    raiFc,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    raiBh,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    raiBc,    -- (Category,Seman,ComName) -> (Category,Seman,ComName) -> (Category, Tag, Seman, ComName, Act)
    ) where
...
appB cate1 cate2
    ...
    | isAvail && ca1 == getCateFromString "s/*s" = (leftCate ca2, "<", semComb se2 se1, "MQ", True)
    ...
    | isAvail && cateEqual ca2 (getCateFromString "np\\.np") = (leftCate ca2, "<", semComb se2 se1, "HnC", True)
    | isAvail && cateEqual ca2 (getCateFromString "(s\\.np)\\x(s\\.np)") = (leftCate ca2, "<", semComb se2 se1, "HvC", True)
    | isAvail && cateEqual ca2 (getCateFromString "(np\\.np)\\.(np\\.np)") = (leftCate ca2, "<", semComb se2 se1, "HaC", True)
    | isAvail && (cateEqual ca2 (getCateFromString "(np/*np)\\*np") || cateEqual ca2 (getCateFromString "(np/*np)\\*(np/.np)") || cateEqual ca2 (getCateFromString "(np/*np)\\*(s/.np)") || cateEqual ca2 (getCateFromString "(np/*np)\\*(s\\.np)")) = (leftCate ca2, "<", semComb se2 se1, "U1P", True)
    | isAvail && cateEqual ca2 (getCateFromString "((s\\.np)/#(s\\.np))\\*(np/.np)") = (leftCate ca2, "<", semComb se2 se1, "U2P", True)
...
comFh2 cate1 cate2
    ...
    se2 = snd3 cate2
    lCate2 = leftCate ca2
    isAvail = (midSlash ca1 == "/#" || midSlash ca1 == "/.") && (midSlash lCate2 == "/#" || midSlash lCate2 == "/.") && rightCate ca1 == leftCate lCate2
